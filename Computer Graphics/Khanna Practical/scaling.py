...
x, y = map(float, input('Enter Scales: ').split())
...
if len(argv) == 2 and argv[1] == 'draw':
    ...
    gcf().canvas.set_window_title('2D Scaling on an Object')
    xlabel('X - Axis')
    ylabel('Y - Axis')
    ...
    plot([px[0], px[n - 1]], [py[0], py[n - 1]], '-', color = 'green', label = 'Before Scaling')
    plot([res_x[0], res_x[n - 1]], [res_y[0], res_y[n - 1]], '-', color = 'red', label = 'After Scaling')
